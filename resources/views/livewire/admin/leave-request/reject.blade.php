<div>
    <x-modal :title="__('Tolak Pengajuan Cuti (HR)')" wire size="2xl">
        @if ($leaveRequest)
            <div class="space-y-6">
                {{-- Request Summary --}}
                <x-card>
                ...
                @if($leaveRequest->manager_approved_at)
                    <x-alert color="green" icon="check-circle">
                        <strong>Manager telah menyetujui</strong> pengajuan ini pada {{ $leaveRequest->manager_approved_at->format('d M Y H:i') }}
                        oleh {{ $leaveRequest->manager?->name }}.
                    </x-alert>
                @endif
                ...
                <form id="reject-leave-{{ $leaveRequest->id }}" wire:submit="reject" class="space-y-4">
                    <div>
                        <x-textarea 
                            ...
                            hint="Jelaskan alasan penolakan dari perspektif HR (minimal 10 karakter, maksimal 500 karakter)"
                            rows="4"
                            placeholder="Contoh: Tidak sesuai dengan kebijakan cuti perusahaan..."
                            required
                        />
                    </div>
                </form>
