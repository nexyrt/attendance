<div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Persetujuan Cuti (HR)</h1>
    </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-5 gap-4">
        ...
        <x-card class="text-center">
            <div class="text-2xl font-bold text-yellow-600 dark:text-yellow-400">
                {{ $this->statusStats['pending_hr'] }}
            </div>
            <div class="text-sm text-gray-600 dark:text-gray-400">Menunggu HR</div>
        </x-card>

        <x-card class="text-center">
            <div class="text-2xl font-bold text-green-600 dark:text-green-400">
                {{ $this->statusStats['approved_by_hr'] }}
            </div>
            <div class="text-sm text-gray-600 dark:text-gray-400">Disetujui HR</div>
        </x-card>
        ...
        <x-table :$headers :$sort :rows="$this->rows" paginate filter loading :quantity="[5, 10, 15, 25]">
            ...
            @interact('column_action', $row)
                <div class="flex gap-1">
                    ...
                    @if($row->status === App\Models\LeaveRequest::STATUS_PENDING_HR)
    ...
    {{-- Modals --}}
    <livewire:admin.leave-request.detail @updated="$refresh" />
    <livewire:admin.leave-request.approve @approved="$refresh" />
    <livewire:admin.leave-request.reject @rejected="$refresh" />
</div>