<div>
    ...
    <div class="mb-6">
        ...
        <p class="text-gray-600">Monitor your team's attendance and performance</p>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div class="bg-white rounded-lg shadow p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-sm text-gray-600">Total Attendances</p>
                    <p class="text-2xl font-bold text-gray-900">{{ $this->monthlyStats['total_attendances'] }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-bold text-gray-900">{{ $this->monthlyStats['attendance_rate'] }}%</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-sm text-gray-600">Punctuality Rate</p>
                    <p class="text-2xl font-bold text-gray-900">{{ $this->monthlyStats['punctuality_rate'] }}%</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            <div class="flex items-center">
                <div class="p-2 bg-purple-100 rounded-lg">
                    <x-icon name="clock" class="w-6 h-6 text-purple-600" />
                </div>
                <div class="ml-4">
                    <p class="text-sm text-gray-600">Total Hours</p>
                    <p class="text-2xl font-bold text-gray-900">{{ $this->monthlyStats['total_working_hours'] }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow mb-6 p-6">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {{-- Performance Table --}}
            <div>
                <h4 class="text-md font-medium text-gray-700 mb-3">Individual Performance</h4>
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="divide-y divide-gray-200">
                            ...
                            <tr>
                                <td class="px-4 py-2 text-sm text-gray-900">{{ $performance['user']->name }}</td>
                                <td class="px-4 py-2">
                                    <span class="inline-flex px-2 py-1 text-xs font-semibold rounded-full
                                        @if($performance['attendance_rate'] >= 90) bg-green-100 text-green-800
                                        @elseif($performance['attendance_rate'] >= 80) bg-yellow-100 text-yellow-800
                                        @else bg-red-100 text-red-800 @endif">
                                        {{ $performance['attendance_rate'] }}%
                                    </span>
                                </td>
                                <td class="px-4 py-2 text-sm text-gray-900">{{ $performance['total_hours'] }}</td>
                            </tr>
                            @endforeach
                        </tbody>
                    </table>
        ...
        <x-table :headers="$headers" :rows="$this->rows" :sort="$sort" paginate filter loading>
            ...
            <x-slot:header>
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                    <x-select.styled label="Team Member" 
                                     :options="$this->getMemberOptions()" 
                                     wire:model.live="member" />
                    
                    <x-input type="date" label="Specific Date" wire:model.live="date" />
                    
                    <x-input type="month" label="Month" wire:model.live="month" />
                    
                    <x-select.styled label="Status" 
            ...
            @interact('column_user', $row)
                <div class="flex items-center">
                    @if($row->user->image)
                        <img class="w-8 h-8 rounded-full mr-3" src="{{ Storage::url($row->user->image) }}" alt="{{ $row->user->name }}">
                    @else
                        <div class="w-8 h-8 bg-gray-300 rounded-full mr-3 flex items-center justify-center">
                            <span class="text-xs font-medium text-gray-700">{{ substr($row->user->name, 0, 2) }}</span>
                        </div>
                    @endif
                    <div>
            ...
            @interact('column_date', $row)
                <div class="text-sm text-gray-900">
                    ...
                    <div class="text-xs text-gray-500">{{ $row->date->format('l') }}</div>
                </div>
            @endinteract

            @interact('column_check_in', $row)
                @if($row->check_in)
                    <span class="text-sm text-gray-900">{{ $row->check_in->format('H:i') }}</span>
                @else
            ...
            @interact('column_check_out', $row)
                @if($row->check_out)
                    <span class="text-sm text-gray-900">{{ $row->check_out->format('H:i') }}</span>
                @else
                    <span class="text-xs text-gray-400">-</span>
                @endif
            @endinteract
            ...
            @interact('column_working_hours', $row)
                @if($row->working_hours)
                    <span class="text-sm text-gray-900">{{ number_format($row->working_hours, 2) }}h</span>
                @else
                    <span class="text-xs text-gray-400">-</span>
                @endif
            @endinteract
            ...
            @interact('column_late_hours', $row)
                @if($row->late_hours)
                    <span class="text-sm text-red-600">{{ number_format($row->late_hours, 2) }}h</span>
                @else
                    <span class="text-xs text-gray-400">-</span>
                @endif
            @endinteract
            ...
            @interact('column_office', $row)
                <div class="text-sm">
                    @if($row->checkInOffice)
                        <div class="text-gray-900">{{ $row->checkInOffice->name }}</div>
                    @endif
                    @if($row->checkOutOffice && $row->checkOutOffice->id !== $row->checkInOffice?->id)
                        <div class="text-xs text-gray-500">Out: {{ $row->checkOutOffice->name }}</div>
                    @endif
                </div>
            @endinteract
        </x-table>
