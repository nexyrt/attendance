<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {{-- Total Kehadiran --}}
        <x-card class="bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-900/20 dark:to-blue-800/20 border-blue-200 dark:border-blue-700">
        ...
        {{-- Hadir Tepat Waktu --}}
        <x-card class="bg-gradient-to-br from-green-50 to-green-100 dark:from-green-900/20 dark:to-green-800/20 border-green-200 dark:border-green-700">
            <div class="flex items-center justify-between">
                <div>
        ...
        {{-- Terlambat --}}
        <x-card class="bg-gradient-to-br from-yellow-50 to-yellow-100 dark:from-yellow-900/20 dark:to-yellow-800/20 border-yellow-200 dark:border-yellow-700">
            <div class="flex items-center justify-between">
                <div>
        ...
        {{-- Pulang Cepat --}}
        <x-card class="bg-gradient-to-br from-orange-50 to-orange-100 dark:from-orange-900/20 dark:to-orange-800/20 border-orange-200 dark:border-orange-700">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm font-medium text-orange-600 dark:text-orange-400">Pulang Cepat</p>
                    <p class="text-2xl font-bold text-orange-900 dark:text-orange-100">{{ $this->stats['early_leave'] }}</p>
                </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div>
                <x-select.styled 
                    ...
                    :options="$this->departments->map(fn($dept) => [
                        'label' => $dept->name,
                        'value' => $dept->id
                    ])->toArray()"
                />
    ...
    <x-card>
        @if($this->rows->total() > 0)
            ...
            @interact('column_department', $row)
                @if($row->user && $row->user->department)
                    <div class="flex items-center space-x-2">
                        <div class="w-3 h-3 rounded-full {{ match($row->user->department->name) {
            ...
            @interact('column_working_hours', $row)
                @if($row->working_hours)
                    <div class="text-sm">
                        <x-badge 
                            :color="$row->working_hours >= 8 ? 'green' : 'orange'"
                            :text="number_format($row->working_hours, 1) . ' jam'"
                        />
                    </div>
                @else
                    <span class="text-gray-400 italic">-</span>
        ...
        @else
            <div class="text-center py-12">
                ...
                <p class="text-gray-500 dark:text-gray-500 text-sm mt-2">Belum ada data kehadiran untuk periode yang dipilih</p>
            </div>
        @endif
    </x-card>
</div>