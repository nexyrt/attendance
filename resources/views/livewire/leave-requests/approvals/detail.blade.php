<div>
    <x-modal :title="__('Detail Pengajuan Cuti')" wire center size="4xl">
        @if ($leaveRequest)
            <div class="space-y-6">
                ...
                {{-- Approval Timeline --}}
                <x-card>
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Status Persetujuan</h3>
                    <div class="space-y-4">
                        {{-- Manager Approval --}}
                        ...
                        <div class="flex items-center space-x-3 p-3 rounded-lg bg-gray-50 dark:bg-gray-700">
                            ...
                            <div>
                                ...
                                @elseif($leaveRequest->status === 'pending_hr')
                                    <div class="text-sm text-yellow-600 dark:text-yellow-400">Menunggu persetujuan HR</div>
                                @else
                                    <div class="text-sm text-gray-500 dark:text-gray-400">Belum diproses</div>
                                @endif
                            </div>
                        ...
                        <div class="flex items-center space-x-3 p-3 rounded-lg bg-gray-50 dark:bg-gray-700">
                            ...
                            <div>
                                ...
                                @elseif($leaveRequest->status === 'pending_director')
                                    <div class="text-sm text-yellow-600 dark:text-yellow-400">Menunggu persetujuan direktur</div>
                                @else
                                    <div class="text-sm text-gray-500 dark:text-gray-400">Belum diproses</div>
                                @endif
                            </div>
        ...
        <x-slot:footer>
            <div class="flex justify-between w-full">
                ...
                @if ($leaveRequest && $leaveRequest->status === App\Models\LeaveRequest::STATUS_PENDING_MANAGER)
...
<script>
    document.addEventListener('livewire:init', () => {
        Livewire.on('print-leave-request', (event) => {
            window.open(`/manager/leave-requests/${event[0].id}/print`, '_blank');
        });
    });
</script>